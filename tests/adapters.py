...
import os
import sys
from typing import IO, Any, BinaryIO, Type, Callable, Literal
...
def run_linear(
    ...
    from llm.transformer import Linear
...
def run_embedding(
    ...
    """
    from llm.transformer import Embedding
...
def run_swiglu(
    ...
    """
    from llm.transformer import SwiGlu

    swiglu = SwiGlu(d_model, d_ff, d_model)
...
def run_multihead_self_attention(
    ...
    """
    from llm.transformer import MultiHeadAttention
    import torch

    att = MultiHeadAttention(d_model=d_model, num_head=num_heads)

    project_weight = torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
    att.project.w.data = project_weight
    att.out_linear.w.data = o_proj_weight
...
def run_rope(
    ...
    from llm.transformer import RoPE

    rope = RoPE(dim=d_k, theta=theta, max_seq_len=max_seq_len)
...
def run_transformer_block(
    ...
    ffn1_weight = weights["ffn.w1.weight"]
    ...
    ts.ffe.w1.w.data = ffn1_weight
    ts.ffe.w2.w.data = ffn2_weight
    ts.ffe.w3.w.data = ffn3_weight

    ts.rms_norm1.g.data = ln1_weight
    ts.rms_norm2.g.data = ln2_weight

    return ts(in_features)
...
def run_transformer_lm(
    ...
    ts.norm.g.data = ln_final_weight
    ts.out_linear.w.data = lm_head_weight
    ts.embedding.embeddings.data = token_embeddings

    for layer in range(num_layers):
        q_proj_weight = weights[f"layers.{layer}.attn.q_proj.weight"]
        k_proj_weight = weights[f"layers.{layer}.attn.k_proj.weight"]
        v_proj_weight = weights[f"layers.{layer}.attn.v_proj.weight"]
        o_proj_weight = weights[f"layers.{layer}.attn.output_proj.weight"]

        ln1_weight = weights[f"layers.{layer}.ln1.weight"]
        ln2_weight = weights[f"layers.{layer}.ln2.weight"]

        ffn1_weight = weights[f"layers.{layer}.ffn.w1.weight"]
        ...
        ts.blocks[layer].ffe.w1.w.data = ffn1_weight
        ts.blocks[layer].ffe.w2.w.data = ffn2_weight
        ts.blocks[layer].ffe.w3.w.data = ffn3_weight

        ts.blocks[layer].rms_norm1.g.data = ln1_weight
        ts.blocks[layer].rms_norm2.g.data = ln2_weight

    return ts(token_ids=in_indices)
...
def run_rmsnorm(
    ...
    """
    from llm.transformer import RmsNorm
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    """
    from llm.transformer import SiLu
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    from llm.transformer import Softmax
...
def run_cross_entropy(
    ...
    """
    from llm.transformer import CrossEntropyLoss
...
def run_gradient_clipping(
    ...
    """
    from llm.transformer import gradient_clip

    gradient_clip(params=parameters, max_norm=max_l2_norm)
...
def get_adamw_cls() -> Any:
    ...
    """
    from llm.transformer import AdamW
...
def run_get_lr_cosine_schedule(
    ...
    """
    from llm.transformer import cos_lr_scheduler

    return cos_lr_scheduler(
        it=it,
        warmup_iters=warmup_iters,
        cos_cycle_iters=cosine_cycle_iters,
...
def run_load_checkpoint(
    ...
    return load_checkpoint(source=src, model=model, optimizer=optimizer)
...
def get_tokenizer(
    ...
    """
    from llm import BpeTokenizer

    tokenizer = BpeTokenizer(special_tokens=special_tokens)
    tokenizer.from_pretrained(vocab, merges)
    return tokenizer
...
def run_compute_group_normalized_rewards(
    ...
    """
    This is a test adapter for the `compute_group_normalized_rewards` function.
    It computes rewards for each group of rollout responses, normalized by the group size.
    """
    from alignment import compute_group_normalized_rewards
...
def run_compute_entropy(logits: torch.Tensor) -> torch.Tensor:
    """This is a test adapter for the `compute_enptropy` function."""
    from alignment import compute_enptropy

    return compute_enptropy(logits)
...
def run_get_response_log_probs(
    ...
    """
    This is a test adapter for the `get_response_log_probs` function.
    It gets the conditional log-probs of the response given the prompt.
    """
    from alignment import get_response_log_probs
...
def run_compute_naive_policy_gradient_loss(
    ...
    """
    This is a test adapter for the `compute_naive_policy_gradient_loss` function.
    It computes the policy gradient loss using either raw rewards or advantages.
    """
    from alignment import compute_naive_policy_gradient_loss

    return compute_naive_policy_gradient_loss(
        advantages_or_raw_reward=raw_rewards_or_advantages,
...
def run_compute_grpo_clip_loss(
    ...
    """
    This is a test adapter for the `compute_grpo_clip_loss` function.
    It computes the GRPO-Clip loss.
    """
    from alignment import compute_grpo_clip_loss
...
def run_compute_policy_gradient_loss(
    ...
    """
    This is a test adapter for the `compute_policy_gradient_loss` function.
    It delegates to the appropriate policy gradient loss function.
    """
    from alignment import compute_policy_gradient_loss
...
def run_masked_mean(
    ...
    """
    This is a test adapter for the `masked_mean` function.
    It computes the mean of the tensor along a dimension, considering only the elements with mask value 1.
    """
    from alignment import masked_mean
...
def run_sft_microbatch_train_step(
    ...
    """This is a test adapter for the `sft_microbatch_train_step` function."""
    from alignment import sft_microbatch_train_step
...
def run_grpo_microbatch_train_step(
    ...
    """
    This is a test adapter for the `grpo_microbatch_train_step` function.
    It computes the policy gradient loss and backprop its gradients for a microbatch.
    """
    from alignment import grpo_microbatch_train_step
...
def run_masked_normalize(
    ...
    """
    This is a test adapter for the `masked_normalize` function.
    It sums over a dimension and normalizes by a constant, considering only the elements with mask value 1.
    """
    from alignment import masked_normalize
